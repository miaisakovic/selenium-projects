...
try:
    wait.until(ExpectedCondition.visibility_of_element_located((By.ID,
                                                                "input-id1")))
    initial_location = driver.find_element(By.ID, "input-id1")
    initial_location.send_keys("Kitchener GO, Kitchener", Keys.DOWN,
                               Keys.RETURN)
...
try:
    wait.until(ExpectedCondition.visibility_of_element_located((By.NAME,
                                                                "date")))
...
try:
    ...
    final_destination.send_keys(Keys.RETURN)
    wait.until(ExpectedCondition.visibility_of_element_located((
                                                        By.ID,
                                                        "TRIP_PLANNER_RESULTS"
                                                      )))
except Exception:
...
for trip in trip_results:
    ...
    try:
        wait.until(ExpectedCondition.element_to_be_clickable((
                                                    By.CLASS_NAME,
                                                    "trip-item-popup-button"
                                                  )))
        ticket_popup = trip.find_element(By.CLASS_NAME,
                                         "trip-item-popup-button")
        ticket_popup.click()
    except Exception:
    ...
    try:
        wait.until(ExpectedCondition.element_to_be_clickable((
                                            By.XPATH,
                                            "//*[text()='Buy E-tickets Now']"
                                          )))
        ticket = driver.find_element(By.XPATH,
                                     "//*[text()='Buy E-tickets Now']")
        ticket.click()
    except Exception:
    ...
    try:
        ...
        close_popup = driver.find_element(By.CLASS_NAME,
                                          "close-button.modal-close-button")
        close_popup.click()
    except Exception:
